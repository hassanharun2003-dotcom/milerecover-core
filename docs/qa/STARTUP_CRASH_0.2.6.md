...
7. **0.2.7-startup.3:** serialize product-ui AsyncStorage writes (latest-wins) and persist outside `setState` updaters (plus post-hydration effect). Rapid onboarding taps / updater-side-effect skips previously left Welcome on disk after in-memory Home.
