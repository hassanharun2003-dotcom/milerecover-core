...
describe('0.2.14 real-device UX simplification', () => {
  ...
  it('build version metadata is 0.2.17', () => {
    ...
    expect(APP_BUILD_LABEL).toMatch(/^0\.2\.17/);
    const config = read('app.config.ts');
    expect(config).toMatch(/version: '0\.2\.17'/);
    expect(config).toMatch(/runtimeVersion: '0\.2\.17'/);
  });
});
