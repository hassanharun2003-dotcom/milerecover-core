...
describe('Production MVP build configuration', () => {
  ...
  it('ships package com.milerecover.app and isolated runtime 0.2.14', () => {
    ...
    expect(appConfigSource).toContain("version: '0.2.14'");
    expect(appConfigSource).toContain("runtimeVersion: '0.2.14'");
  ...
  it('uses isolated preview-foundation channel so prior OTAs cannot replace UI', () => {
    ...
    expect(eas.build.preview.channel).toBe('preview-foundation-0.2.14');
    expect(eas.build.preview.env?.APP_VARIANT).toBe('preview');
    expect(isStandaloneBuild('preview')).toBe(true);
    expect(isStandaloneUpdateChannel('preview-foundation-0.2.14')).toBe(true);
    ...
    expect(packageJson.scripts?.['update:preview']).toContain('preview-foundation-0.2.14');
  });
